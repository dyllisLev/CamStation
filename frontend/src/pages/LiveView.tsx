...
export function LiveView({ onNavigate }: Props) {
  ...
  const handleSeek = (camId: string, ts: number) => {
    ...
    if (!seg) return;
    const segDate = new Intl.DateTimeFormat('en-CA', { timeZone: 'Asia/Seoul' })
      .format(new Date(seg.ts_start * 1000));
...
>{isFullscreen ? '⊠ 종료' : '⛶ 전체화면'}</button>
      ...
      <main style={{ position: 'relative', overflow: 'hidden', paddingBottom: 4, borderBottom: '2px solid #555' }}>
        {gridLayout.length > 0 && (
          <CameraGrid
            cameras={cameras}
