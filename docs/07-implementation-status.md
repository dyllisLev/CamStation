...
## 2026-09-11 recording playback startup (deployed)
...
- Web tests 106/106, lint, web/daemon builds and Chrome/WebKit native playback
  controls passed. Revision `57e0aa645ee388bd6ccdadf36d53014d65ad6011` deployed
  at 18:37 KST; exact CI/OpenShip/image/health, recording continuity and both
  browsers' public-page playback controls passed. macOS Safari itself remains
  unverified. See the
...
- The [completion audit](superpowers/specs/2026-09-09-playback-completion-audit.md) identified two remaining checks. Both have now been addressed: Viewer tests pass 55/55 after reconciling the skill-description contract, and the native management-reconnection fix was packaged as test MSI 2.0.30 and verified on WIN11-DELL. The authorized rollout deployed server revision `47a4ad33bc020e00269c3c2bac6a9880460169dd` and monitoring-PC MSI 2.0.30. CI/OpenShip/image identity, healthy status, DB checks and all eight growing recording indexes passed. The [deployment record](playback-deployment-2026-09-09.md) includes the first rollout incident, fixes, preserved originals and recovery limits.
- Release integration on top of NVENC commit `a447bee` passed the full Go suite, daemon build, Web tests 101/101, Web lint/build, and Viewer tests 55/55 plus build. Embedded assets were regenerated from the combined source. The seven native MSI 2.0.30 source-overlay hashes match the release tree.
- Screen-first analysis now covers browser live/recordings and official Windows Viewer live/recordings: [requirements, events and frontend/backend design](superpowers/specs/2026-09-09-playback-screen-to-backend-analysis.md), [implementation plan](superpowers/plans/2026-09-09-playback-screen-to-backend-plan.md).
- The selected direction is B-style recordings browsing plus synchronized recorded playback in the existing live timeline, reusing existing focus view. Existing zoom/double-click gestures are preserved.
- The B recordings workspace now provides camera/date browsing, an independently scrolling segment list, shared controls/timeline, video zoom/pan, and fullscreen. Operator management remains in its own tab; selecting its playback action reuses the same player. Viewer routes expose playback only.
- The existing live timeline now seeks every displayed camera to one absolute clock. Focus view preserves mode, position, pause and rate; LIVE reconnects the live adapters. PTZ is stopped and disabled during recorded playback, and obsolete asynchronous PTZ-stop completions cannot override a newer seek or LIVE action.
- Recorder output uses fragmented MP4 with packet-epoch PRFT anchors. Only complete, synced fragments are indexed and served through logical HLS/init/fragment URLs. Normal archive moves preserve identity; old MP4 playback and pending-backup protection remain. Archived cameras stay browsable, and segment pagination uses a stable cursor.
- A bounded continuation across small normal file-boundary gaps prevents single-camera playback from stopping between files. Explicit gap seeks keep the requested time; a healthy peer camera's clock is not forcibly advanced.
- Windows preserves an allowed recordings document during management reconnection by reusing the existing navigation allowlist. Test MSI 2.0.30 contains this source change; installed EXE/app bundle hashes match the build, and the installed ASAR predicate was inspected.
- Local validation passed: full Go tests, Web lint/build, daemon build, and Viewer build. Web tests passed 92/93 initially; the one obsolete external-player source assertion was updated for the playback-only Viewer workspace and its focused rerun passed. Viewer tests initially passed 54/55 because a source assertion expected the previous PC-control skill introduction. The assertion now checks the current discovery/target/control contracts; full Viewer tests pass 55/55 on Linux and during Windows MSI build.
- An isolated local daemon reused two existing camera relay streams. Chromium confirmed growing-file HLS decoding, legacy MP4 seek, synchronized live-timeline replay, focus/LIVE transitions, pause/rate preservation, management-to-player routing, gap/next-recording, file continuation, and receiver failure/retry without changing playback intent. New recordings continued and finalized with backup state pending. Browser-rendered Viewer routes share the same implementation.
- The initial Windows baseline check was incomplete: WIN11-DELL had MSI 2.0.24 while the operating NUC had 2.0.29; both EXEs misleadingly reported 2.0.0. The test PC was upgraded to the exact operating 2.0.29 MSI with configuration, private identity and service environment preserved. EXE, app.asar and service hashes now match. The relaunched Viewer connected and accepted direct fullscreen and synchronized recorded-playback input. Direct Windows checks also confirmed focus-view round trips preserve the paused timestamp, LIVE restores both streams, and the B recordings page starts without autoplay, selected segments decode and pause, and native fullscreen exit/re-entry preserves the paused position. No production deployment or monitoring-PC change was performed during that pre-release check.

- On test MSI 2.0.30, a graceful five-second management-service stop/start preserved the same Viewer process and recordings document, selected segment, fullscreen and paused 18:51:04 KST position. A fresh `management_recovered` / `pipe_closed` event confirms actual IPC recovery. Settings, private client identity and service environment survived the MSI update. Control tasks and remote runs were cleaned; monitoring PC was not changed during that pre-release check.
- Viewer navigation is shown once in the common shell. Live layout constraints keep long recorded-time controls inside the available window width, and the shared bridge restores the last native-confirmed fullscreen state across Viewer document navigation. Test-PC native exit/re-entry confirmed this state and the actual title-bar transition. The test Viewer remains connected to the isolated server with two cameras and recording disabled for visual review.
...
- Current monitoring PC Viewer: MSI 2.0.30; installed EXE, app.asar and management-service hashes
  match the validated test MSI. Service Running/Auto; configuration and private client identity
  preserved; the existing eight-camera arrangement remains maximized.
...
- The canary publishes HTTP `18081/tcp` on both management `10.0.0.26` and monitoring
  ...
  and Docker bridge address are not advertised. go2rtc API `1984` and RTSP `8554` remain
  container-internal. Native/non-Docker operation retains local-interface candidate discovery when
  no explicit candidate is configured.
...
- Windows monitoring client delivery:
  ...
  - `scripts/build-viewer-msi.ps1` builds the standard MSI locally, while the Settings release channel and `/api/viewers/app/*` endpoints serve a size/SHA-256-verified exact MSI or the supported legacy Setup EXE without routing MSI artifacts into the EXE-only automatic updater
