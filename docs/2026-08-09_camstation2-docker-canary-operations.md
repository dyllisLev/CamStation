...
- 모니터링 PC Viewer: [http://192.168.0.160:18081/live](http://192.168.0.160:18081/live)
- 모바일·무인 뷰어: [http://192.168.0.160:18081/viewer](http://192.168.0.160:18081/viewer)
- 설정·Windows 설치파일: [http://192.168.0.160:18081/settings](http://192.168.0.160:18081/settings)
- 상태 API: [http://192.168.0.160:18081/api/health](http://192.168.0.160:18081/api/health)
...
모니터링 단말은 `192.168.0.0/24` 주소를 사용한다. 관리 PC에서는 같은 HTTP 서비스의
`http://10.0.0.26:18081` 주소도 사용할 수 있지만, direct WebRTC candidate는 모니터링 LAN의
`192.168.0.160:18555`만 광고한다.
...
| WebRTC candidate | `192.168.0.160:18555` 한 개만 광고; 10 대역과 Docker bridge 주소는 광고하지 않음 |
...
| 서버 주소 입력값 | `http://192.168.0.160:18081` |

운영자가 수행할 수락 순서는 다음과 같다.

1. `http://192.168.0.160:18081/settings`를 열고 `Windows 설치 파일 다운로드`를 선택한다.
...
2026-08-12 13:00 KST에는 테스트 PC의 `192.168.0.178` 인터페이스와 저장된 서버 주소
`http://192.168.0.160:18081`을 사용해 세 카메라가 모두 첫 WebRTC attempt에서
...
검증을 위해 오프라인 Viewer에 새 명령을 억지로 만들지 않았다. 이후 2026-08-12 192 대역
수락에서는 저장 주소 변경 뒤 기존 Viewer가 새 주소를 읽도록 `restart_viewer` 한 건만 실행했고,
전달·확인·실행·성공까지 완료된 것을 확인했다.
...
| `/live` WebRTC 초기 10 대역 시험 | 3/3 첫 attempt 성공, 2.917~4.352초, retry/MSE/stream fallback 0 |
| Windows Viewer 192 대역 시험 | source `192.168.0.178`, 3/3 첫 attempt 성공, 0.712~1.102초, retry/MSE/stream fallback 0 |
| 192 대역 재생 진단 로그 | 세 session의 구조화 event 18건, 금지 URL/SDP/ICE/자격증명 패턴 0 |
| 192 대역 실제 미디어 | 서버 `eth1`에서 테스트 PC→`192.168.0.160:18555/udp` 세 흐름 확인 |
...
그 이미지 전환 전 root 전용 포인터 백업은 `.env.pre-webrtc-20260812-113427.bak`, Compose 백업은
...
1. 모니터링 단말이 `192.168.0.0/24`에 접근 가능한지 확인한다. 관리 단말은 10 대역 HTTP도 쓸 수 있다.
2. 모니터링 주소가 정확히 `http://192.168.0.160:18081/viewer`인지 확인한다. 아직 HTTPS/DNS 경로는 없다.
...
2. host의 `192.168.0.160:18555/tcp+udp` publish와 생성 설정의 candidate
...
- 외부 진입은 10 대역 관리 HTTP와 192 대역 모니터링 HTTP·WebRTC `18555/tcp+udp`뿐이다.
  direct WebRTC는 테스트 PC의 192 인터페이스와 Windows Viewer에서 검증했지만 인터넷
  공개·HTTPS/TLS 환경은 검증하지 않았다.
...
- finding: 이전 약 10초 지연은 camera 장애가 아니라 공개되지 않은 WebRTC candidate를 두 번
  timeout한 뒤 같은 `live` stream의 MSE로 바뀐 결함이었다. 이 최초 수락은 10 대역에서
  수행됐으며, 최종 모니터링 LAN 수락은 E-008에 기록한다.

### E-008 — 192 대역 모니터링 PC direct WebRTC

- observed_at: 2026-08-12 12:51-13:04 KST
- source_type: Docker port/config inspection, source-bound Windows network probe, server packet
  capture, Viewer structured telemetry, Windows interactive GUI capture, public API, recorder growth,
  systemd inspection
- source_ref: WIN11-DELL `192.168.0.178` → server `192.168.0.160`, 설치된 Viewer 2.0.24,
  `playback_event`, Docker runtime
- transport: HTTP `192.168.0.160:18081/tcp`, WebRTC
  `192.168.0.160:18555/tcp+udp` → container `8555`; advertised candidate는
  `192.168.0.160:18555` 한 개이며 10 대역·bridge candidate는 없음
- network_result: source를 `192.168.0.178`로 고정한 HTTP health와 TCP 18081/18555가 성공했고,
  서버 `eth1`에서 해당 source가 18555/udp로 만든 실제 미디어 흐름 세 개를 확인함
- windows_result: 세 카메라 모두 WebRTC attempt 1에서 0.712~1.102초에 재생,
  retry/MSE/stream fallback 0; 세 session의 구조화 event 18건과 금지 URL/SDP/ICE/자격증명
  패턴 0
- gui_result: session 1의 실제 `CamStation 2.0` 창에 영상과 UIA 카메라 control 3개가 있고
  재연결·대체 스트림 문구가 없음을 원본 이미지로 확인; capture task/worker 0으로 정리
- evidence_hashes: Windows PNG
  `57918f52dd239bbc8b22905af2b8c3d14ce0aa3e95f086777044a5fe8bfc40fa`, UIA
  `489ef95db9896e2c07b9418dc275c6c7ed2a4d5fb5df7e880660aa223bc05a6f`, complete JSON
  `e8d43baaa9aff09ed1c3f1138608dcafbcad3a40979d0dfd4979eb058e59e94b`
- continuity: container healthy/restart 0, 10 대역 HTTP health 유지, camera 3/3 streaming,
  recorder 3/3 running, 세 active recording inode 모두 증가, 보안·mount 경계 유지,
  legacy 1.0 five-unit PID/restart baseline 불변
- rollback: 같은 `.11-canary` image와 root 전용
  `.env.pre-monitor-192-20260812-125110.bak`,
  `compose.yaml.pre-monitor-192-20260812-125110.bak` 보존
...
- confidence: high
- resolution: 모니터링 LAN의 충돌 없는 `192.168.0.160:18555/tcp+udp`를 publish하고 같은
  주소의 명시 candidate만 광고했다. transport fallback과 실제 `live`→`focus` stream
  fallback을 분리하고 bounded 구조화 재생 로그를 추가해 테스트 PC의 192 source에서 첫
  attempt 성공과 실제 UDP 유입을 검증했다.

### P-001 — 모바일 영상 경로

- title: 모바일 Viewer 요청부터 집 카메라 MSE 재생까지
- path_type: callflow
- start: `http://192.168.0.160:18081/viewer`
...
잔여 위험은 HTTP/WebRTC의 내부 LAN 직접 접근, 수동 재시작, 집 카메라에 한정된 검증이다. PID 용량은 8대
