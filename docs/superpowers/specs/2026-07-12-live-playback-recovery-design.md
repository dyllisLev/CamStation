...
Once a fallback produces media, it is only a temporary visible route. The tile keeps that working connection untouched while a hidden bounded probe checks the preferred output once per minute, and switches back only after the primary video clock genuinely advances. If no candidate is playing, every exhausted recovery episode waits five seconds before revisiting both primary transports, the fallback, and isolated resubscription. Immediately failing attempts are spaced at least three seconds apart. Five seconds of continuous media progress resets the finite recovery episode; a single frame does not. This supersedes the former 15-second/five-minute cooldown policy; see the [2026-09-07 recovery contract](2026-09-07-live-recovery-latency-design.md).
