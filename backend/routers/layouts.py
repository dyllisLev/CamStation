...
async def list_layouts():
    async with aiosqlite.connect(get_db_path()) as db:
        ...
        cursor = await db.execute(
            "SELECT id, name, data, created_at, updated_at FROM layouts ORDER BY updated_at DESC"
        )
...
async def create_layout(req: CreateLayoutRequest):
    ...
    async with aiosqlite.connect(get_db_path()) as db:
        await db.execute(
            ...
            (layout_id, req.name.strip(), data_json, now, now),
        )
        await db.commit()
    return LayoutProfile(id=layout_id, name=req.name.strip(), data=req.data, created_at=now, updated_at=now)
...
async def update_layout(layout_id: str, req: UpdateLayoutRequest):
    async with aiosqlite.connect(get_db_path()) as db:
        db.row_factory = aiosqlite.Row
        cursor = await db.execute(
            "SELECT id, name, data, created_at FROM layouts WHERE id = ?", (layout_id,)
        )
        ...
        await db.execute(
            ...
            (new_name, new_data, now, layout_id),
        )
        await db.commit()
